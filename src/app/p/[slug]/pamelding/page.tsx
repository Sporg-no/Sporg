    ...
    <div className="min-h-screen bg-sporg-bg text-sporg-text">
      <nav className="border-b border-sporg-border bg-sporg-surface-2">
        <div className="mx-auto flex max-w-2xl items-center justify-between px-6 py-4">
          <Link href={`/p/${params.slug}`} className="text-sm text-sporg-text-2 hover:text-sporg-text">
            ← Tilbake til {arr.tittel}
          </Link>
          <Link href="/" className="font-bold text-sporg-accent">Sporg</Link>
        </div>
      </nav>

      <div className="mx-auto max-w-2xl px-6 py-10">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-sporg-text">{arr.tittel}</h1>
          <div className="mt-2 flex items-center gap-4 text-sm text-sporg-text-3">
            <span>📅 {formatDatoTid(arr.startDato)}</span>
            <span>📍 {arr.sted}</span>
            <span className="font-medium text-sporg-text">{formatPris(arr.pris)}</span>
          </div>
        </div>
        ...
        {erPameldt ? (
          <div className="rounded-xl border border-green-500/20 bg-green-500/10 p-8 text-center">
            ...
            <h2 className="text-lg font-semibold text-green-400">Du er allerede påmeldt!</h2>
          ...
          <div className="rounded-xl border border-sporg-border bg-sporg-surface-2 p-8 text-center shadow-card">
            <h2 className="font-semibold text-sporg-text">Påmelding er stengt</h2>
            <p className="mt-2 text-sm text-sporg-text-3">
              {arr.pameldingsFrist && new Date() > arr.pameldingsFrist
